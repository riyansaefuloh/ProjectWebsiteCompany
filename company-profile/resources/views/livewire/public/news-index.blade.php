<div>
    <div style="background: #e2e8f0; padding: 40px; text-align: center; margin-bottom: 30px;">
        <h1>News & Articles</h1>
        <p>Stay updated with our latest company news and industry insights.</p>
    ...
    <div style="margin-bottom: 20px; text-align: right;">
        <input type="text" wire:model.live.debounce.300ms="search" placeholder="Search news..." style="padding: 8px; width: 300px; max-width: 100%;">
    </div>
    ...
    <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 30px; margin-bottom: 40px;">
        @forelse($news as $article)
            <div style="border: 1px solid #ddd; padding: 15px; border-radius: 6px; background: white;">
                ...
                @else
                    <div style="width: 100%; height: 200px; background: #eee; display: flex; align-items: center; justify-content: center; border-radius: 4px;">No Image</div>
                @endif
                ...
                <div style="font-size: 12px; color: #888; margin-bottom: 10px; display: flex; gap: 15px;">
                    ...
                    <span>👤 {{ $article->author ? $article->author->name : 'Admin' }}</span>
                </div>
                
                <p style="font-size: 14px; color: #666; margin-bottom: 15px;">{{ Str::limit($article->translated_excerpt, 100) }}</p>
                <a href="{{ route('news.show', $article->slug) }}" style="display: inline-block; padding: 8px 15px; background: #2563eb; color: white; text-decoration: none; border-radius: 4px; font-size: 14px;">Read Article</a>
            </div>
        @empty
